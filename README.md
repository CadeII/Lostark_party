...
- 캐릭터/각인은 전체 매칭에서 1번만 쓰고 끝나는 것이 아닙니다.
  - 사용 기록은 `owner + character name + className + build + raid family` 기준입니다.
  - 따라서 같은 캐릭터/각인도 `cathedral`, `serka`, `end`를 각각 1회씩 갈 수 있습니다.
  - 단, 같은 family 안에서는 한 번만 사용됩니다. 예: `cathedral_1`, `cathedral_2`, `cathedral_3` 중 하나를 가면 같은 캐릭터/각인으로 cathedral family를 다시 가지 않습니다.
- 화면 상단의 `캐릭터별 잔여 레이드`에서 각 캐릭터가 남긴 `지평의 성당`, `세르카`, `종막` 상태를 확인할 수 있습니다. 파티에 들어간 캐릭터는 해당 항목에 취소선이 표시됩니다.
- `레이드 family 전역 1회 제한` 옵션을 켜면 구버전처럼 `cathedral`, `serka`, `end` family를 전체 매칭에서 한 번만 사용합니다. 기본값은 OFF입니다.
...
- `consumeRaidFamily`: `false`가 기본값입니다. 꺼져 있으면 같은 raid 종류도 다른 캐릭터 조합이 남아 있는 동안 계속 매칭 후보로 남습니다.
- `trackCharacterUsageByRaidFamily`: 캐릭터 사용 기록을 raid family 단위로 관리합니다. 켜져 있으면 같은 캐릭터/각인이 `지평의 성당`, `세르카`, `종막`을 각각 갈 수 있습니다.
...
- 캐릭터 사용 기록 범위: `trackCharacterUsageByRaidFamily`
