...
function createDiagnostics(raid, selectedCharacters, eligibleCharacters, allowExternalSlot) {
...
export function matchRaid(raid, selectedOwnerNames, characters, classMeta, options = {}) {
  const opts = mergeOptions(options);
  ...
  const selectedCharacters = enrichedCharacters.filter((character) => isSelectable(character, selectedOwners));
  ...
  const diagnostics = createDiagnostics(raid, selectedCharacters, eligibleCharacters, allowExternalSlot);
...
export function findBestMatch({ raids, selectedOwners, characters, classMeta, usedRaidFamilies = [], options = {} }) {
  const opts = mergeOptions(options);
  ...
  for (const raid of raids) {
    ...
    const result = matchRaid(raid, selectedOwners, characters, classMeta, opts);
...
export function removeMatchedRaidFromQueue(raidQueue, matchedRaid, options = {}) {
  ...
  return raidQueue.filter((raid) => raid.key !== matchedRaid.key);
}

export function autoMatchAll({ raids, selectedOwners, characters, classMeta, options = {} }) {
  const opts = mergeOptions(options);
  const matches = [];
  let queue = [...raids];
  const usedRaidFamilies = [];
  const failuresByRound = [];

  while (queue.length > 0) {
    const result = findBestMatch({ raids: queue, selectedOwners, characters, classMeta, usedRaidFamilies, options: opts });
    if (!result.ok) {
    ...
    usedRaidFamilies.push(result.match.raidFamily);
    queue = removeMatchedRaidFromQueue(queue, result.match.raid, opts);
  }

  return {
    matches,
    remainingRaids: queue,
    usedRaidFamilies,
    failuresByRound,
  };
}
