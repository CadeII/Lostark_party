...
const state = {
  ...
  raidQueue: [...RAIDS],
  matched: [],
  // 캐릭터 사용 기록은 "캐릭터/각인 + raid family" 단위로 저장한다.
  // 따라서 같은 캐릭터도 cathedral, serka, end에는 각각 한 번씩 매칭될 수 있다.
  usedCharacterRaidKeys: new Set(),
  usedRaidFamilies: new Set(),
  options: {
...
const elements = {
  ...
  characterRaidStatus: document.querySelector("#characterRaidStatus"),
};
...
const RAID_FAMILY_LABELS = Object.freeze({
  cathedral: "지평의 성당",
  serka: "세르카",
  end: "종막",
});

const RAID_FAMILY_ORDER = [...new Set(RAIDS.map((raid) => getRaidFamily(raid.key)))].map((family) => ({
  family,
  label: RAID_FAMILY_LABELS[family] ?? family,
}));

function getCharacterRaidProgress(character) {
  return RAID_FAMILY_ORDER.map(({ family, label }) => {
    const usageKey = getCharacterRaidUsageKey(character, family);
    const used = usageKey ? state.usedCharacterRaidKeys.has(usageKey) : false;
...
function hasAnyRecordedRaidUsage(character) {
  return getCharacterRaidProgress(character).some((item) => item.used);
}

function syncAvailableRaidQueue(baseRaids = RAIDS) {
  state.raidQueue = getAvailableRaids({
    raids: baseRaids,
    selectedOwners: [...state.selectedOwners],
    characters: CHARACTERS,
    classMeta: CLASS_META,
    usedRaidFamilies: [...state.usedRaidFamilies],
    usedCharacterRaidKeys: [...state.usedCharacterRaidKeys],
...
function renderOwners() {
  ...
  elements.ownerList.querySelectorAll("input").forEach((input) => {
    input.addEventListener("change", () => {
      ...
      syncAvailableRaidQueue();
      renderSelectedOwnerText();
      renderRaidQueue();
      renderCharacterSummary();
      renderCharacterRaidStatus();
    });
  });
...
function renderCharacterSummary() {
  ...
  const usedCharacterRaidCount = state.usedCharacterRaidKeys.size;
  ...
  elements.characterSummary.innerHTML = `
    ...
    <div class="metric"><strong>그룹별</strong><span>캐릭터 사용 단위</span></div>
    <div class="metric"><strong>${usedCharacterRaidCount}</strong><span>사용 기록</span></div>
  `;
}

function renderCharacterRaidStatus() {
  const enriched = enrichCharacters(CHARACTERS, CLASS_META);
  const selectedOwners = state.selectedOwners;
  const visibleCharacters = enriched
    .filter((character) => selectedOwners.has(character.owner) || (character.reserve && hasAnyRecordedRaidUsage(character)))
    .sort((a, b) => {
      const ownerOrder = a.owner.localeCompare(b.owner, "ko");
      if (ownerOrder !== 0) return ownerOrder;
      return b.level - a.level || a.name.localeCompare(b.name, "ko");
    });

  if (visibleCharacters.length === 0) {
    elements.characterRaidStatus.innerHTML = `<div class="empty">선택된 owner의 캐릭터가 없습니다.</div>`;
    return;
  }

  elements.characterRaidStatus.innerHTML = visibleCharacters
    .map((character) => {
      const progress = getCharacterRaidProgress(character);
      const completed = progress.filter((item) => item.used).length;
      const allDone = completed === progress.length;
      const reserveBadge = character.reserve ? `<span class="badge reserve">reserve</span>` : "";

      return `
        <article class="character-raid-row ${allDone ? "all-done" : ""}">
          <div class="character-raid-name">
            <strong>${character.name}</strong>
            <span>${character.owner} · ${character.className}/${character.build} · Lv.${character.level}</span>
            ${reserveBadge}
          </div>
          <div class="raid-family-status-list">
            ${progress
              .map((item) => `
                <span class="raid-status-item ${item.used ? "used" : "remaining"}" data-family="${item.family}">
          ...
          <div class="character-raid-count">${completed}/${progress.length}</div>
        </article>
      `;
    })
    .join("");
    ...
    <section class="match-card">
      <header class="match-header">
        <div>
          ...
          <p>${raid.key} · ${raid.partySize}인 · family ${match.raidFamily} · Lv.${raid.minLevel}~${raid.maxLevel === 9999 ? "∞" : raid.maxLevel}</p>
        </div>
        <div class="score-box">
          ...
          .map((failure) => {
            ...
            return `
              <li>
                ...
                <span class="muted">후보 ${d.eligibleCharacterCount}명, 딜러 ${d.eligibleDps}, 서폿 ${d.eligibleSupports}, 레벨 제외 ${d.levelFiltered}, 같은 레이드 그룹 사용 제외 ${d.usedCharacterRaidFiltered ?? 0}, meta 누락 ${d.missingMeta}, 공석 허용 ${d.allowExternalSlot ? "ON" : "OFF"}</span>
              </li>
            `;
          })
          .join("")}
...
function rememberCharacterRaidUsage(match) {
  for (const key of getMatchCharacterRaidUsageKeys(match)) {
    state.usedCharacterRaidKeys.add(key);
  }
}
...
function runMatchOne() {
  ...
  rememberCharacterRaidUsage(result.match);
  state.usedRaidFamilies.add(result.match.raidFamily);
  syncAvailableRaidQueue(RAIDS);
  renderDiagnostics(result.failures ?? []);
  render();
}
...
function runMatchAll() {
  ...
  state.usedRaidFamilies = new Set(result.usedRaidFamilies);
  state.usedCharacterRaidKeys = new Set(result.usedCharacterRaidKeys);
...
function resetQueue() {
  state.usedRaidFamilies = new Set();
  syncAvailableRaidQueue(RAIDS);
  renderDiagnostics([]);
  render();
}
...
function resetResults() {
  state.matched = [];
  state.usedCharacterRaidKeys = new Set();
  state.usedRaidFamilies = new Set();
  syncAvailableRaidQueue(RAIDS);
  renderDiagnostics([]);
  render();
